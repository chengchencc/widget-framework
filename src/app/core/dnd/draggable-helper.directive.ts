...
export class DraggableHelperDirective implements OnInit, OnDestroy {
  ...
  private onDragStart(event: WidgetDragEvent): void {
    ...
    this.startPosition = {
      x: event.event.clientX - clientRect.left,
      y: event.event.clientY - clientRect.top
    };
  ...
  private onDragMove(event: WidgetDragEvent): void {
    ...
    // position the helper...
    this.positionStrategy.left(`${event.event.clientX - this.startPosition.x}px`);
    this.positionStrategy.top(`${event.event.clientY - this.startPosition.y}px`);
